/**
 * Adds product_variations.price if missing (fixes INSERT from admin when 002 ran without this column).
 *
 * Run: npm run db:migrate-variation-price
 */
const fs = require('fs');
...
async function runMigration() {
  ...
  try {
    const sqlPath = path.join(__dirname, 'migrations', '003_add_product_variations_price.sql');
    const sql = fs.readFileSync(sqlPath, 'utf8');
    console.log('Running migration: migrations/003_add_product_variations_price.sql');
