...
import { getInstSQCWebGLRenderer } from './webglRenderer';
...
function getEngine(): EngineState | null {
  ...
  if (!getInstSQCWebGLRenderer()) return null;
...
export function subscribeInstSQCPreview(
...
export const INST_SQC_FILTER_ID = 'film';

export function isInstSQCFilter(filterId: string): boolean {
  return filterId === INST_SQC_FILTER_ID;
}
